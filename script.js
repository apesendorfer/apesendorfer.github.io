...
function f3(i) {
    ...
    if (output_list[i].length > 0) {
        for (let j = 0; j < output_list[i].length; j++) {
            text2 += "<button style=\"background: #E9F2FD; border-radius: 10px; margin-right: 5px; margin-botton: 5px;\" id=\"b" + j + "\" type=\"button\" onclick=\"f4(this.innerHTML.substring(3), " + i + ")\">" + (j+1) + ". " + output_list[i][j] + "</button>"
        }
    }
